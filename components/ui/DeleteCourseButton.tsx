...
export default function DeleteCourseButton({
  ...
  const handleDelete = async () => {
    ...
    try {
      // Verify the acting user is the teacher before allowing the write.
      // RLS also enforces teacher-only deletes server-side.
      const { data: authData } = await supabase.auth.getUser();
      const user = authData?.user;

      if (!user) {
        setToast({ type: "error", text: "You must be signed in to delete a course." });
        setDeleting(false);
        return;
      }
      ...
      if (error) {
        setToast({ type: "error", text: `Failed to delete course: ${error.message}` });
        setDeleting(false);
        return;
      }

      setOpen(false);
      setToast({ type: "success", text: "Course deleted successfully." });
      router.refresh();
    } catch (err: unknown) {
      setToast({
        type: "error",
        ...
        >
          ...
          >
            ...
            <p className="text-slate-400 text-xs mb-6">
              This will also delete all lectures in this course. This action cannot be undone.
            </p>
