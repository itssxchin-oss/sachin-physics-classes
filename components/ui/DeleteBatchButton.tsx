...
export default function DeleteBatchButton({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!user) {
        setToast({ type: "error", text: "You must be signed in to delete a batch." });
        setDeleting(false);
        return;
      }
      ...
      if (error) {
        setToast({ type: "error", text: `Failed to delete batch: ${error.message}` });
        setDeleting(false);
        return;
      }

      setOpen(false);
      setToast({ type: "success", text: "Batch deleted successfully." });
      router.refresh();
    } catch (err: unknown) {
      setToast({
        type: "error",
    ...
    <>
      ...
      {open && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Delete batch confirmation"
          onClick={() => {
        ...
        >
          ...
          >
            ...
            <h2 className="text-xl font-bold text-white mb-2">Delete Batch?</h2>
            <p className="text-slate-300 text-sm leading-relaxed mb-2">
              Are you sure you want to delete{" "}
              <span className="text-white font-semibold">&quot;{batchTitle}&quot;</span>?
            </p>
            <p className="text-slate-400 text-xs mb-6">
              This will also delete all subjects, chapters, and lectures inside this batch. This action cannot be undone.
            </p>
