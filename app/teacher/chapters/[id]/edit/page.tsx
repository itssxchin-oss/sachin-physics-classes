...
export default function EditChapterPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        if (courseList && !cancelled) {
          setCourses(courseList as { id: string; title: string }[]);
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!user || user.email !== TEACHER_EMAIL) {
        setError("Unauthorized: only the teacher account can edit chapters.");
        setSaving(false);
        return;
      }
      ...
      if (updateError) {
        setError(`Failed to update chapter: ${updateError.message}`);
        setSaving(false);
        return;
      }

      router.push("/teacher/dashboard?updated=chapter");
      router.refresh();
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "An unexpected error occurred.");
      setSaving(false);
      ...
      <main className="flex-1 max-w-3xl mx-auto px-4 sm:px-6 py-24 w-full">
        ...
        <div className="glass p-8 md:p-10 rounded-3xl border border-white/10 shadow-2xl fade-up">
          <div className="mb-8">
            ...
            <h1 className="text-3xl font-extrabold text-white">Edit Chapter</h1>
            <p className="text-slate-400 text-sm mt-1">
              Update chapter details, order number, or course assignment.
            </p>
          </div>
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-slate-400 text-sm">Loading chapter details...</p>
            </div>
          )}
          ...
          {!loading && unauthorized && (
            <div className="mb-6 p-4 rounded-xl bg-red-500/20 border border-red-500/30 text-red-200 text-sm text-center font-medium">
              Unauthorized: only the teacher account can edit chapters.
            </div>
          )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                {coursesLoading ? (
                  ...
                  <select
                    id="edit-chapter-course-dropdown"
                    required
                    value={courseId}
                    onChange={(e) => setCourseId(e.target.value)}
                    className="w-full px-4 py-3 rounded-xl bg-slate-900 border border-white/20 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer"
                  >
                    {courses.map((course) => (
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  id="edit-chapter-order-input"
                  type="number"
                  min={1}
                  required
                  value={orderNumber}
                  onChange={(e) => setOrderNumber(Number(e.target.value))}
                  className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  id="edit-chapter-desc-input"
                  rows={3}
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="Overview of topics covered in this chapter..."
                  className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              ...
              <div className="pt-4 flex items-center justify-end gap-4">
                ...
                <button
                  id="update-chapter-btn"
                  type="submit"
                  disabled={saving}
                  className="px-8 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm btn-glow transition-all disabled:opacity-50"
                >
