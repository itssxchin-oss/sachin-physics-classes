...
export default function LecturePage({ params }: LecturePageProps) {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (!error && lectureData) {
          ...
          // 3. Resolve chapter -> subject -> batch chain for breadcrumb & back-link
        ...
        if (currentUserId) {
          ...
          const rec = progressData as Progress | null;
          if (rec?.completed) setIsCompleted(true);
        }
      } catch (err) {
  ...
  const handleToggleComplete = async () => {
    ...
    try {
      if (userId) {
        ...
        const { error } = await (supabase.from("progress") as any).upsert(payload, {
  ...
  // Back link resolves once batch/subject are loaded; fallback to my-batches
  const backHref =
    batchId && subjectId
        ...
        <div className="flex items-center gap-2 flex-shrink-0 relative">
          ...
          {showMenu && (
            <div className="absolute right-0 top-12 w-56 rounded-2xl glass border border-white/10 bg-slate-900/95 backdrop-blur-2xl shadow-2xl p-2 z-50 fade-up space-y-1">
              ...
              <button
                onClick={() => { handleToggleFullscreen(); setShowMenu(false); }}
                className="w-full px-3 py-2.5 rounded-xl text-left text-xs font-semibold text-slate-200 hover:bg-white/10 flex items-center gap-2.5 transition-colors"
              >
        ...
        >
          {loading ? (
            <div className="aspect-video w-full flex flex-col items-center justify-center text-slate-400 bg-slate-900/80">
              <div className="animate-spin w-10 h-10 border-[3px] border-blue-500 border-t-transparent rounded-full mb-4" />
              <p className="text-sm font-medium text-slate-300">Preparing video player...</p>
            </div>
          ) : lecture ? (
            ...
            <div className="aspect-video w-full flex flex-col items-center justify-center p-8 text-center bg-slate-900">
              ...
              <p className="text-slate-400 text-sm max-w-md mb-6">
                This lecture could not be loaded or may have been removed.
              </p>
              <Link href={backHref} className="px-6 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition-all">
                Back to Subject
              </Link>
            </div>
          )}
